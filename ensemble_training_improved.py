...
class ImprovedMultiDiseaseEnsemble:
    def __init__(self):
        self.models = {}
        self.scalers = {}
        self.feature_names = {}
    ...
    def load_and_preprocess_kidney(self):
        """Load and preprocess kidney disease dataset WITHOUT leaking features"""
        df = pd.read_csv('data/kidney/kidney_disease.csv')
        
        # Convert target to binary
        df['class'] = (df['class'] == 'ckd').astype(int)
        
        # REMOVE LEAKING FEATURES - these are medical conditions that directly indicate CKD
        leaking_features = ['rbc', 'pc', 'pcc', 'ba', 'htn', 'dm', 'cad', 'apppe', 'ane']
        df = df.drop(columns=leaking_features, errors='ignore')
        
        print(f"Removed leaking features: {leaking_features}")
    ...
    def train_disease_model(self, disease_name):
        ...
        # Apply SMOTE for imbalanced datasets (only for training data)
        if disease_name in ['heart', 'kidney']:
...
if __name__ == "__main__":
    ...
    ensemble = ImprovedMultiDiseaseEnsemble()
